...
export async function refresh(refreshToken: string) {
  ...
  return sequelize.transaction(async (t) => {
    ...
    if (!stored) {
      const alreadyRevoked = await RefreshToken.findOne({
        where: {
          userId: payload.userId,
          tokenHash,
        },
      ...
      if (alreadyRevoked) {
        ...
        await writeAuditLog({
          ...
          outcome: 'failure',
          severity: 'warning',
          metadata: { reason: 'revoked_token_reuse_detected', tokenHash },
        });
        throw new AppError(401, 'Compromised refresh token detected. All sessions revoked.', 'TOKEN_COMPROMISED');
      }
...
export async function revokeDevice(userId: string, deviceId: string) {
  return sequelize.transaction(async (t) => {
    ...
    await RefreshToken.update(
      { revokedAt: new Date() },
      { where: { userId, deviceId, revokedAt: null }, transaction: t }
    );

    await device.destroy({ transaction: t });

    await writeAuditLog({
      action: AuditAction.AUTH_DEVICE_REVOKE,
